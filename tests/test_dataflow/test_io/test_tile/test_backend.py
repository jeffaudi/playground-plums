...
class TestImport:
    ...
    def test_all_fail(self):
        with pytest.raises(ImportError, match='Error importing plums.dataflow.io: '
            ...
            with disable_import('PIL', 'lycon', 'cv2', 'plums.dataflow.io.tile._vendor.turbojpeg'):
                import plums.dataflow.io.tile._backend  # noqa: F401

    def test_pillow_fail(self):
        with disable_import('PIL'):
            import plums.dataflow.io.tile._backend
            assert not plums.dataflow.io.tile._backend._HAS_PILLOW
            assert plums.dataflow.io.tile._backend._HAS_LYCON
            assert plums.dataflow.io.tile._backend._HAS_CV2
            assert plums.dataflow.io.tile._backend._HAS_TURBO_JPEG

    def test_lycon_fail(self):
        with disable_import('lycon'):
            import plums.dataflow.io.tile._backend
            assert plums.dataflow.io.tile._backend._HAS_PILLOW
            assert not plums.dataflow.io.tile._backend._HAS_LYCON
            assert plums.dataflow.io.tile._backend._HAS_CV2
            assert plums.dataflow.io.tile._backend._HAS_TURBO_JPEG
    ...
    def test_onpen_cv_fail(self):
        with disable_import('cv2'):
            ...
            assert plums.dataflow.io.tile._backend._HAS_PILLOW
            assert plums.dataflow.io.tile._backend._HAS_LYCON
            assert not plums.dataflow.io.tile._backend._HAS_CV2
            assert plums.dataflow.io.tile._backend._HAS_TURBO_JPEG

    def test_turbo_jpeg_fail(self):
        with disable_import('plums.dataflow.io.tile._vendor.turbojpeg'):
            import plums.dataflow.io.tile._backend
            assert plums.dataflow.io.tile._backend._HAS_PILLOW
            assert plums.dataflow.io.tile._backend._HAS_LYCON
            assert plums.dataflow.io.tile._backend._HAS_CV2
            assert not plums.dataflow.io.tile._backend._HAS_TURBO_JPEG
...
class TestLoad:
    ...
    @pytest.mark.parametrize('disabled_backend', (('none', ), ('plums.dataflow.io.tile._vendor.turbojpeg', ),
                                                  ...
                                                  ('plums.dataflow.io.tile._vendor.turbojpeg', 'lycon', 'cv2'),
                                                  ('lycon', 'cv2'),
                                                  ('plums.dataflow.io.tile._vendor.turbojpeg', 'cv2'),
                                                  ('plums.dataflow.io.tile._vendor.turbojpeg', 'lycon', 'PIL'),
                                                  ('plums.dataflow.io.tile._vendor.turbojpeg', 'PIL'),
                                                  ('lycon', 'cv2', 'PIL'),
                                                  ('lycon', 'PIL'),
                                                  ('cv2', 'PIL')), ids=lambda backends: ', '.join(backends))
    def test_jpeg_ext(self, disabled_backend):
        with disable_import(*disabled_backend):
            from plums.dataflow.io.tile._backend import _load_jpg
    ...
    def test_jpeg_no_backend(self):
        with disable_import('plums.dataflow.io.tile._vendor.turbojpeg', 'lycon', 'cv2'):
            import plums.dataflow.io.tile._backend
            plums.dataflow.io.tile._backend._HAS_PILLOW = False  # Disable Pillow after import to avoid early raise

            with pytest.raises(RuntimeError, match='No backend available to open JPG image.'):
                plums.dataflow.io.tile._backend._load_jpg(Path(__file__)[:-1] / '_data/test_jpg.jpg')
    ...
    def test_png_no_backend(self):
        with disable_import('lycon', 'cv2', 'PIL'):
            ...
            with pytest.raises(RuntimeError, match='No backend available to open PNG image.'):
                plums.dataflow.io.tile._backend._load_png(Path(__file__)[:-1] / '_data/test_png.png')

            with pytest.raises(RuntimeError, match='No backend available to open PNG image.'):
                plums.dataflow.io.tile._backend._load_png(Path(__file__)[:-1] / '_data/test_png')
    ...
    def test_png_jpg(self, disabled_backend):
        with disable_import(*disabled_backend):
            from plums.dataflow.io.tile._backend import _load_png, _load_jpg
...
class TestDump:
    ...
    def test_dump_jpg(self, disabled_backend, jpeg_image, tmp_path):
        ...
        with disable_import(*disabled_backend):
            from plums.dataflow.io.tile._backend import _dump_jpg, _load_jpg
    ...
    def test_dump_png(self, disabled_backend, jpeg_image, tmp_path):
        ...
        with disable_import(*disabled_backend):
            from plums.dataflow.io.tile._backend import _dump_png, _load_png
...
class TestImage:
    ...
    def test_load(self, disabled_backend, image):
        with disable_import(*disabled_backend):
            from plums.dataflow.io.tile._backend import Image, _load_jpg, _load_png
