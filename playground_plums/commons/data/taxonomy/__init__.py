...
class Taxonomy(Tree):
    ...
    * Everything which is a *spam* **is** a *canned-meat* and **is** a *meat*, **but not the reciprocal**.
    ...
    Those relationships allow for set-aware |Label| lookups (as in :meth:`~playground_plums.commons.Tree.get` or
    :meth:`~playground_plums.commons.Tree.__getitem__`), tree iteration (as in
    :meth:`~playground_plums.commons.Tree.iterate`) and |Label| tuple validation against self's taxonomy.
    ...
    *true-roots* |Tree| as attributes and :attr:`~playground_plums.commons.PropertyContainer.properties`:
